// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import DropboxOutlinedSvg from '@ant-design/icons-svg/asn/DropboxOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const DropboxOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={DropboxOutlinedSvg} />;

 /**![dropbox](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(DropboxOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'DropboxOutlined';
}

export default RefIcon;