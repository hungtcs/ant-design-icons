// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FastBackwardFilledSvg from '@ant-design/icons-svg/asn/FastBackwardFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FastBackwardFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FastBackwardFilledSvg} />;

 /**![fast-backward](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FastBackwardFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FastBackwardFilled';
}

export default RefIcon;