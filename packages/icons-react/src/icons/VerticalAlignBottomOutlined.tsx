// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import VerticalAlignBottomOutlinedSvg from '@ant-design/icons-svg/asn/VerticalAlignBottomOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const VerticalAlignBottomOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={VerticalAlignBottomOutlinedSvg} />;

 /**![vertical-align-bottom](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(VerticalAlignBottomOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'VerticalAlignBottomOutlined';
}

export default RefIcon;