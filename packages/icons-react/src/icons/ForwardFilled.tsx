// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import ForwardFilledSvg from '@ant-design/icons-svg/asn/ForwardFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const ForwardFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={ForwardFilledSvg} />;

 /**![forward](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(ForwardFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'ForwardFilled';
}

export default RefIcon;