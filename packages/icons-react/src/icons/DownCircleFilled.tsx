// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import DownCircleFilledSvg from '@ant-design/icons-svg/asn/DownCircleFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const DownCircleFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={DownCircleFilledSvg} />;

 /**![down-circle](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(DownCircleFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'DownCircleFilled';
}

export default RefIcon;