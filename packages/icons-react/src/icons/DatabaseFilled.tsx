// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import DatabaseFilledSvg from '@ant-design/icons-svg/asn/DatabaseFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const DatabaseFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={DatabaseFilledSvg} />;

 /**![database](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(DatabaseFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'DatabaseFilled';
}

export default RefIcon;