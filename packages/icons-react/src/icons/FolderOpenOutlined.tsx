// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FolderOpenOutlinedSvg from '@ant-design/icons-svg/asn/FolderOpenOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FolderOpenOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FolderOpenOutlinedSvg} />;

 /**![folder-open](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FolderOpenOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FolderOpenOutlined';
}

export default RefIcon;