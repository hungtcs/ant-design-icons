// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import PullRequestOutlinedSvg from '@ant-design/icons-svg/asn/PullRequestOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const PullRequestOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={PullRequestOutlinedSvg} />;

 /**![pull-request](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(PullRequestOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'PullRequestOutlined';
}

export default RefIcon;