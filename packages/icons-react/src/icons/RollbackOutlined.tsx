// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import RollbackOutlinedSvg from '@ant-design/icons-svg/asn/RollbackOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const RollbackOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={RollbackOutlinedSvg} />;

 /**![rollback](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(RollbackOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'RollbackOutlined';
}

export default RefIcon;