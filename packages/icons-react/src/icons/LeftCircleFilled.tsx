// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import LeftCircleFilledSvg from '@ant-design/icons-svg/asn/LeftCircleFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const LeftCircleFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={LeftCircleFilledSvg} />;

 /**![left-circle](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(LeftCircleFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'LeftCircleFilled';
}

export default RefIcon;