// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import GooglePlusOutlinedSvg from '@ant-design/icons-svg/asn/GooglePlusOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const GooglePlusOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={GooglePlusOutlinedSvg} />;

 /**![google-plus](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(GooglePlusOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'GooglePlusOutlined';
}

export default RefIcon;