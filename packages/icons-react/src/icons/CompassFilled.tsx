// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import CompassFilledSvg from '@ant-design/icons-svg/asn/CompassFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const CompassFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={CompassFilledSvg} />;

 /**![compass](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(CompassFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'CompassFilled';
}

export default RefIcon;