// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FileExclamationTwoToneSvg from '@ant-design/icons-svg/asn/FileExclamationTwoTone';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FileExclamationTwoTone = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FileExclamationTwoToneSvg} />;

 /**![file-exclamation](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FileExclamationTwoTone);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FileExclamationTwoTone';
}

export default RefIcon;