// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import ShoppingTwoToneSvg from '@ant-design/icons-svg/asn/ShoppingTwoTone';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const ShoppingTwoTone = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={ShoppingTwoToneSvg} />;

 /**![shopping](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(ShoppingTwoTone);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'ShoppingTwoTone';
}

export default RefIcon;