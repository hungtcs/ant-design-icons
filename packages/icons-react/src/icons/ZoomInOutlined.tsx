// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import ZoomInOutlinedSvg from '@ant-design/icons-svg/asn/ZoomInOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const ZoomInOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={ZoomInOutlinedSvg} />;

 /**![zoom-in](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(ZoomInOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'ZoomInOutlined';
}

export default RefIcon;