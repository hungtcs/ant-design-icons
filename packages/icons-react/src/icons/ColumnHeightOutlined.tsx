// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import ColumnHeightOutlinedSvg from '@ant-design/icons-svg/asn/ColumnHeightOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const ColumnHeightOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={ColumnHeightOutlinedSvg} />;

 /**![column-height](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(ColumnHeightOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'ColumnHeightOutlined';
}

export default RefIcon;