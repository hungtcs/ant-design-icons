// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import MedicineBoxFilledSvg from '@ant-design/icons-svg/asn/MedicineBoxFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const MedicineBoxFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={MedicineBoxFilledSvg} />;

 /**![medicine-box](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(MedicineBoxFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'MedicineBoxFilled';
}

export default RefIcon;