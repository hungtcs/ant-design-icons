// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import DeleteColumnOutlinedSvg from '@ant-design/icons-svg/asn/DeleteColumnOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const DeleteColumnOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={DeleteColumnOutlinedSvg} />;

 /**![delete-column](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(DeleteColumnOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'DeleteColumnOutlined';
}

export default RefIcon;