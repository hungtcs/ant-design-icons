// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FastForwardOutlinedSvg from '@ant-design/icons-svg/asn/FastForwardOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FastForwardOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FastForwardOutlinedSvg} />;

 /**![fast-forward](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FastForwardOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FastForwardOutlined';
}

export default RefIcon;