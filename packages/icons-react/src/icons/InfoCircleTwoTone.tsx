// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import InfoCircleTwoToneSvg from '@ant-design/icons-svg/asn/InfoCircleTwoTone';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const InfoCircleTwoTone = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={InfoCircleTwoToneSvg} />;

 /**![info-circle](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(InfoCircleTwoTone);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'InfoCircleTwoTone';
}

export default RefIcon;