// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import DeleteOutlinedSvg from '@ant-design/icons-svg/asn/DeleteOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const DeleteOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={DeleteOutlinedSvg} />;

 /**![delete](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(DeleteOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'DeleteOutlined';
}

export default RefIcon;