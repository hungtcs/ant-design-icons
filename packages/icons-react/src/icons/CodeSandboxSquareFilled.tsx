// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import CodeSandboxSquareFilledSvg from '@ant-design/icons-svg/asn/CodeSandboxSquareFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const CodeSandboxSquareFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={CodeSandboxSquareFilledSvg} />;

 /**![code-sandbox-square](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(CodeSandboxSquareFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'CodeSandboxSquareFilled';
}

export default RefIcon;