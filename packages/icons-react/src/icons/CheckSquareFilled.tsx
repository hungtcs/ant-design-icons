// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import CheckSquareFilledSvg from '@ant-design/icons-svg/asn/CheckSquareFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const CheckSquareFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={CheckSquareFilledSvg} />;

 /**![check-square](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(CheckSquareFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'CheckSquareFilled';
}

export default RefIcon;