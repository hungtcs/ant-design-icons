// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import LayoutTwoToneSvg from '@ant-design/icons-svg/asn/LayoutTwoTone';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const LayoutTwoTone = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={LayoutTwoToneSvg} />;

 /**![layout](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(LayoutTwoTone);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'LayoutTwoTone';
}

export default RefIcon;