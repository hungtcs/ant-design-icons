// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import DingtalkOutlinedSvg from '@ant-design/icons-svg/asn/DingtalkOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const DingtalkOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={DingtalkOutlinedSvg} />;

 /**![dingtalk](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(DingtalkOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'DingtalkOutlined';
}

export default RefIcon;