// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import InsertRowAboveOutlinedSvg from '@ant-design/icons-svg/asn/InsertRowAboveOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const InsertRowAboveOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={InsertRowAboveOutlinedSvg} />;

 /**![insert-row-above](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(InsertRowAboveOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'InsertRowAboveOutlined';
}

export default RefIcon;