// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FileWordFilledSvg from '@ant-design/icons-svg/asn/FileWordFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FileWordFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FileWordFilledSvg} />;

 /**![file-word](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FileWordFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FileWordFilled';
}

export default RefIcon;