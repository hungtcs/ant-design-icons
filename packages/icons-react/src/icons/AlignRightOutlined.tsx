// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import AlignRightOutlinedSvg from '@ant-design/icons-svg/asn/AlignRightOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const AlignRightOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={AlignRightOutlinedSvg} />;

 /**![align-right](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(AlignRightOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'AlignRightOutlined';
}

export default RefIcon;