// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import SwitcherFilledSvg from '@ant-design/icons-svg/asn/SwitcherFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const SwitcherFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={SwitcherFilledSvg} />;

 /**![switcher](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(SwitcherFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'SwitcherFilled';
}

export default RefIcon;