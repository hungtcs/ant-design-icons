// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import SortAscendingOutlinedSvg from '@ant-design/icons-svg/asn/SortAscendingOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const SortAscendingOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={SortAscendingOutlinedSvg} />;

 /**![sort-ascending](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(SortAscendingOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'SortAscendingOutlined';
}

export default RefIcon;