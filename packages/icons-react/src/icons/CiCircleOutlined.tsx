// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import CiCircleOutlinedSvg from '@ant-design/icons-svg/asn/CiCircleOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const CiCircleOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={CiCircleOutlinedSvg} />;

 /**![ci-circle](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(CiCircleOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'CiCircleOutlined';
}

export default RefIcon;