// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FileExcelOutlinedSvg from '@ant-design/icons-svg/asn/FileExcelOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FileExcelOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FileExcelOutlinedSvg} />;

 /**![file-excel](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FileExcelOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FileExcelOutlined';
}

export default RefIcon;