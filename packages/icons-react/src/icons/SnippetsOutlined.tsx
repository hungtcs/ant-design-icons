// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import SnippetsOutlinedSvg from '@ant-design/icons-svg/asn/SnippetsOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const SnippetsOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={SnippetsOutlinedSvg} />;

 /**![snippets](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(SnippetsOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'SnippetsOutlined';
}

export default RefIcon;