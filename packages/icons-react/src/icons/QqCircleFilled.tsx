// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import QqCircleFilledSvg from '@ant-design/icons-svg/asn/QqCircleFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const QqCircleFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={QqCircleFilledSvg} />;

 /**![qq-circle](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(QqCircleFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'QqCircleFilled';
}

export default RefIcon;