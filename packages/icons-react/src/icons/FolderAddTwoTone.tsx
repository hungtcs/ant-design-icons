// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FolderAddTwoToneSvg from '@ant-design/icons-svg/asn/FolderAddTwoTone';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FolderAddTwoTone = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FolderAddTwoToneSvg} />;

 /**![folder-add](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FolderAddTwoTone);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FolderAddTwoTone';
}

export default RefIcon;