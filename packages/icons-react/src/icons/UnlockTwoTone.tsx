// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import UnlockTwoToneSvg from '@ant-design/icons-svg/asn/UnlockTwoTone';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const UnlockTwoTone = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={UnlockTwoToneSvg} />;

 /**![unlock](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(UnlockTwoTone);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'UnlockTwoTone';
}

export default RefIcon;