// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import PieChartFilledSvg from '@ant-design/icons-svg/asn/PieChartFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const PieChartFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={PieChartFilledSvg} />;

 /**![pie-chart](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(PieChartFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'PieChartFilled';
}

export default RefIcon;