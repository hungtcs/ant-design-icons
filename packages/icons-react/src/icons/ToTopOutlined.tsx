// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import ToTopOutlinedSvg from '@ant-design/icons-svg/asn/ToTopOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const ToTopOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={ToTopOutlinedSvg} />;

 /**![to-top](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(ToTopOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'ToTopOutlined';
}

export default RefIcon;