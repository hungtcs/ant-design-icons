// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import TwitchOutlinedSvg from '@ant-design/icons-svg/asn/TwitchOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const TwitchOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={TwitchOutlinedSvg} />;

 /**![twitch](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(TwitchOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'TwitchOutlined';
}

export default RefIcon;