// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FastForwardFilledSvg from '@ant-design/icons-svg/asn/FastForwardFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FastForwardFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FastForwardFilledSvg} />;

 /**![fast-forward](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FastForwardFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FastForwardFilled';
}

export default RefIcon;