// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import InsertRowBelowOutlinedSvg from '@ant-design/icons-svg/asn/InsertRowBelowOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const InsertRowBelowOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={InsertRowBelowOutlinedSvg} />;

 /**![insert-row-below](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(InsertRowBelowOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'InsertRowBelowOutlined';
}

export default RefIcon;