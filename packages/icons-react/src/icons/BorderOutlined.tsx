// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import BorderOutlinedSvg from '@ant-design/icons-svg/asn/BorderOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const BorderOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={BorderOutlinedSvg} />;

 /**![border](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(BorderOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'BorderOutlined';
}

export default RefIcon;