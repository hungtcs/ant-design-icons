// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import TwitterOutlinedSvg from '@ant-design/icons-svg/asn/TwitterOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const TwitterOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={TwitterOutlinedSvg} />;

 /**![twitter](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(TwitterOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'TwitterOutlined';
}

export default RefIcon;