// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import LinkedinFilledSvg from '@ant-design/icons-svg/asn/LinkedinFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const LinkedinFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={LinkedinFilledSvg} />;

 /**![linkedin](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(LinkedinFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'LinkedinFilled';
}

export default RefIcon;