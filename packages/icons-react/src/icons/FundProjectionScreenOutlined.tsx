// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FundProjectionScreenOutlinedSvg from '@ant-design/icons-svg/asn/FundProjectionScreenOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FundProjectionScreenOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FundProjectionScreenOutlinedSvg} />;

 /**![fund-projection-screen](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FundProjectionScreenOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FundProjectionScreenOutlined';
}

export default RefIcon;