// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import EnterOutlinedSvg from '@ant-design/icons-svg/asn/EnterOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const EnterOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={EnterOutlinedSvg} />;

 /**![enter](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(EnterOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'EnterOutlined';
}

export default RefIcon;