// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import WarningTwoToneSvg from '@ant-design/icons-svg/asn/WarningTwoTone';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const WarningTwoTone = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={WarningTwoToneSvg} />;

 /**![warning](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(WarningTwoTone);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'WarningTwoTone';
}

export default RefIcon;