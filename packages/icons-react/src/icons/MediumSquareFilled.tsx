// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import MediumSquareFilledSvg from '@ant-design/icons-svg/asn/MediumSquareFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const MediumSquareFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={MediumSquareFilledSvg} />;

 /**![medium-square](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(MediumSquareFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'MediumSquareFilled';
}

export default RefIcon;