// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import PercentageOutlinedSvg from '@ant-design/icons-svg/asn/PercentageOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const PercentageOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={PercentageOutlinedSvg} />;

 /**![percentage](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(PercentageOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'PercentageOutlined';
}

export default RefIcon;