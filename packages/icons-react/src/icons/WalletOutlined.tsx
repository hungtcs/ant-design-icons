// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import WalletOutlinedSvg from '@ant-design/icons-svg/asn/WalletOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const WalletOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={WalletOutlinedSvg} />;

 /**![wallet](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(WalletOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'WalletOutlined';
}

export default RefIcon;