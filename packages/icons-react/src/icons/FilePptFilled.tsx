// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FilePptFilledSvg from '@ant-design/icons-svg/asn/FilePptFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FilePptFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FilePptFilledSvg} />;

 /**![file-ppt](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FilePptFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FilePptFilled';
}

export default RefIcon;