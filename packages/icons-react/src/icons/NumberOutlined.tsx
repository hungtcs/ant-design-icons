// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import NumberOutlinedSvg from '@ant-design/icons-svg/asn/NumberOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const NumberOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={NumberOutlinedSvg} />;

 /**![number](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(NumberOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'NumberOutlined';
}

export default RefIcon;