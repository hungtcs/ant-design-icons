// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import DingtalkSquareFilledSvg from '@ant-design/icons-svg/asn/DingtalkSquareFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const DingtalkSquareFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={DingtalkSquareFilledSvg} />;

 /**![dingtalk-square](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(DingtalkSquareFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'DingtalkSquareFilled';
}

export default RefIcon;