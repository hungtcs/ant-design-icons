// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import VerticalAlignMiddleOutlinedSvg from '@ant-design/icons-svg/asn/VerticalAlignMiddleOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const VerticalAlignMiddleOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={VerticalAlignMiddleOutlinedSvg} />;

 /**![vertical-align-middle](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(VerticalAlignMiddleOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'VerticalAlignMiddleOutlined';
}

export default RefIcon;