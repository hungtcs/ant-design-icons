// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import InfoCircleFilledSvg from '@ant-design/icons-svg/asn/InfoCircleFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const InfoCircleFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={InfoCircleFilledSvg} />;

 /**![info-circle](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(InfoCircleFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'InfoCircleFilled';
}

export default RefIcon;