// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import CrownOutlinedSvg from '@ant-design/icons-svg/asn/CrownOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const CrownOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={CrownOutlinedSvg} />;

 /**![crown](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(CrownOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'CrownOutlined';
}

export default RefIcon;