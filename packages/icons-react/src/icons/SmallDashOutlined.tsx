// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import SmallDashOutlinedSvg from '@ant-design/icons-svg/asn/SmallDashOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const SmallDashOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={SmallDashOutlinedSvg} />;

 /**![small-dash](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(SmallDashOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'SmallDashOutlined';
}

export default RefIcon;