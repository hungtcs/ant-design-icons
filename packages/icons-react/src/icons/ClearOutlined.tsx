// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import ClearOutlinedSvg from '@ant-design/icons-svg/asn/ClearOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const ClearOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={ClearOutlinedSvg} />;

 /**![clear](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(ClearOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'ClearOutlined';
}

export default RefIcon;