// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import CopyrightCircleFilledSvg from '@ant-design/icons-svg/asn/CopyrightCircleFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const CopyrightCircleFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={CopyrightCircleFilledSvg} />;

 /**![copyright-circle](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(CopyrightCircleFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'CopyrightCircleFilled';
}

export default RefIcon;