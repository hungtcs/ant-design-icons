// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FastBackwardOutlinedSvg from '@ant-design/icons-svg/asn/FastBackwardOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FastBackwardOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FastBackwardOutlinedSvg} />;

 /**![fast-backward](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FastBackwardOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FastBackwardOutlined';
}

export default RefIcon;