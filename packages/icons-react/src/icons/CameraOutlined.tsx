// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import CameraOutlinedSvg from '@ant-design/icons-svg/asn/CameraOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const CameraOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={CameraOutlinedSvg} />;

 /**![camera](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(CameraOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'CameraOutlined';
}

export default RefIcon;