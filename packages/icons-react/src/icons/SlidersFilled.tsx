// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import SlidersFilledSvg from '@ant-design/icons-svg/asn/SlidersFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const SlidersFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={SlidersFilledSvg} />;

 /**![sliders](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(SlidersFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'SlidersFilled';
}

export default RefIcon;