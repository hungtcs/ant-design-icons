// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import DingdingOutlinedSvg from '@ant-design/icons-svg/asn/DingdingOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const DingdingOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={DingdingOutlinedSvg} />;

 /**![dingding](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(DingdingOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'DingdingOutlined';
}

export default RefIcon;