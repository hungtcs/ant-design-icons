// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FileWordOutlinedSvg from '@ant-design/icons-svg/asn/FileWordOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FileWordOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FileWordOutlinedSvg} />;

 /**![file-word](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FileWordOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FileWordOutlined';
}

export default RefIcon;