// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import CheckSquareOutlinedSvg from '@ant-design/icons-svg/asn/CheckSquareOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const CheckSquareOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={CheckSquareOutlinedSvg} />;

 /**![check-square](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(CheckSquareOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'CheckSquareOutlined';
}

export default RefIcon;