// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import ExclamationOutlinedSvg from '@ant-design/icons-svg/asn/ExclamationOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const ExclamationOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={ExclamationOutlinedSvg} />;

 /**![exclamation](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(ExclamationOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'ExclamationOutlined';
}

export default RefIcon;