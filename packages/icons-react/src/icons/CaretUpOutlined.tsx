// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import CaretUpOutlinedSvg from '@ant-design/icons-svg/asn/CaretUpOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const CaretUpOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={CaretUpOutlinedSvg} />;

 /**![caret-up](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(CaretUpOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'CaretUpOutlined';
}

export default RefIcon;