// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FontColorsOutlinedSvg from '@ant-design/icons-svg/asn/FontColorsOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FontColorsOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FontColorsOutlinedSvg} />;

 /**![font-colors](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FontColorsOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FontColorsOutlined';
}

export default RefIcon;