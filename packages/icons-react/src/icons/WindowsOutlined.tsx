// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import WindowsOutlinedSvg from '@ant-design/icons-svg/asn/WindowsOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const WindowsOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={WindowsOutlinedSvg} />;

 /**![windows](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(WindowsOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'WindowsOutlined';
}

export default RefIcon;