// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FormatPainterOutlinedSvg from '@ant-design/icons-svg/asn/FormatPainterOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FormatPainterOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FormatPainterOutlinedSvg} />;

 /**![format-painter](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FormatPainterOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FormatPainterOutlined';
}

export default RefIcon;