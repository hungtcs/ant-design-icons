// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FolderAddOutlinedSvg from '@ant-design/icons-svg/asn/FolderAddOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FolderAddOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FolderAddOutlinedSvg} />;

 /**![folder-add](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FolderAddOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FolderAddOutlined';
}

export default RefIcon;