// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import AreaChartOutlinedSvg from '@ant-design/icons-svg/asn/AreaChartOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const AreaChartOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={AreaChartOutlinedSvg} />;

 /**![area-chart](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(AreaChartOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'AreaChartOutlined';
}

export default RefIcon;