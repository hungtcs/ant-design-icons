// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import DotNetOutlinedSvg from '@ant-design/icons-svg/asn/DotNetOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const DotNetOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={DotNetOutlinedSvg} />;

 /**![dot-net](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(DotNetOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'DotNetOutlined';
}

export default RefIcon;