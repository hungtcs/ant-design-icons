// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import PicCenterOutlinedSvg from '@ant-design/icons-svg/asn/PicCenterOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const PicCenterOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={PicCenterOutlinedSvg} />;

 /**![pic-center](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(PicCenterOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'PicCenterOutlined';
}

export default RefIcon;