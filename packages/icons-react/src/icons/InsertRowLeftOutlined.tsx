// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import InsertRowLeftOutlinedSvg from '@ant-design/icons-svg/asn/InsertRowLeftOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const InsertRowLeftOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={InsertRowLeftOutlinedSvg} />;

 /**![insert-row-left](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(InsertRowLeftOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'InsertRowLeftOutlined';
}

export default RefIcon;