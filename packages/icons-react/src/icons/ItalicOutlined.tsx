// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import ItalicOutlinedSvg from '@ant-design/icons-svg/asn/ItalicOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const ItalicOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={ItalicOutlinedSvg} />;

 /**![italic](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(ItalicOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'ItalicOutlined';
}

export default RefIcon;