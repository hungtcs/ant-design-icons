// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import BorderOuterOutlinedSvg from '@ant-design/icons-svg/asn/BorderOuterOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const BorderOuterOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={BorderOuterOutlinedSvg} />;

 /**![border-outer](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(BorderOuterOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'BorderOuterOutlined';
}

export default RefIcon;