// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import RightCircleFilledSvg from '@ant-design/icons-svg/asn/RightCircleFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const RightCircleFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={RightCircleFilledSvg} />;

 /**![right-circle](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(RightCircleFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'RightCircleFilled';
}

export default RefIcon;