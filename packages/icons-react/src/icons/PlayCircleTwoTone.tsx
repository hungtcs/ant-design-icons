// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import PlayCircleTwoToneSvg from '@ant-design/icons-svg/asn/PlayCircleTwoTone';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const PlayCircleTwoTone = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={PlayCircleTwoToneSvg} />;

 /**![play-circle](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(PlayCircleTwoTone);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'PlayCircleTwoTone';
}

export default RefIcon;