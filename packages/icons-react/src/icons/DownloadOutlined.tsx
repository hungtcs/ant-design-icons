// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import DownloadOutlinedSvg from '@ant-design/icons-svg/asn/DownloadOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const DownloadOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={DownloadOutlinedSvg} />;

 /**![download](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(DownloadOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'DownloadOutlined';
}

export default RefIcon;