// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import GoogleCircleFilledSvg from '@ant-design/icons-svg/asn/GoogleCircleFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const GoogleCircleFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={GoogleCircleFilledSvg} />;

 /**![google-circle](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(GoogleCircleFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'GoogleCircleFilled';
}

export default RefIcon;