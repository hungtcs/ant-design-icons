// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import SketchOutlinedSvg from '@ant-design/icons-svg/asn/SketchOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const SketchOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={SketchOutlinedSvg} />;

 /**![sketch](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(SketchOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'SketchOutlined';
}

export default RefIcon;