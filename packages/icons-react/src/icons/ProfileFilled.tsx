// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import ProfileFilledSvg from '@ant-design/icons-svg/asn/ProfileFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const ProfileFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={ProfileFilledSvg} />;

 /**![profile](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(ProfileFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'ProfileFilled';
}

export default RefIcon;