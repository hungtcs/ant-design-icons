// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import DownSquareTwoToneSvg from '@ant-design/icons-svg/asn/DownSquareTwoTone';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const DownSquareTwoTone = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={DownSquareTwoToneSvg} />;

 /**![down-square](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(DownSquareTwoTone);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'DownSquareTwoTone';
}

export default RefIcon;