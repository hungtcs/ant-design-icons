// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import MenuUnfoldOutlinedSvg from '@ant-design/icons-svg/asn/MenuUnfoldOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const MenuUnfoldOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={MenuUnfoldOutlinedSvg} />;

 /**![menu-unfold](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(MenuUnfoldOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'MenuUnfoldOutlined';
}

export default RefIcon;