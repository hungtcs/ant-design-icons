// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import PictureFilledSvg from '@ant-design/icons-svg/asn/PictureFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const PictureFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={PictureFilledSvg} />;

 /**![picture](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(PictureFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'PictureFilled';
}

export default RefIcon;