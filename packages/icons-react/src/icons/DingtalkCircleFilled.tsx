// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import DingtalkCircleFilledSvg from '@ant-design/icons-svg/asn/DingtalkCircleFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const DingtalkCircleFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={DingtalkCircleFilledSvg} />;

 /**![dingtalk-circle](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(DingtalkCircleFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'DingtalkCircleFilled';
}

export default RefIcon;