// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import ScissorOutlinedSvg from '@ant-design/icons-svg/asn/ScissorOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const ScissorOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={ScissorOutlinedSvg} />;

 /**![scissor](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(ScissorOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'ScissorOutlined';
}

export default RefIcon;