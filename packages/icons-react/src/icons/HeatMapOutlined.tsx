// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import HeatMapOutlinedSvg from '@ant-design/icons-svg/asn/HeatMapOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const HeatMapOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={HeatMapOutlinedSvg} />;

 /**![heat-map](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(HeatMapOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'HeatMapOutlined';
}

export default RefIcon;