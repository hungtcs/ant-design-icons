// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import ProjectOutlinedSvg from '@ant-design/icons-svg/asn/ProjectOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const ProjectOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={ProjectOutlinedSvg} />;

 /**![project](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(ProjectOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'ProjectOutlined';
}

export default RefIcon;