// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import AntDesignOutlinedSvg from '@ant-design/icons-svg/asn/AntDesignOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const AntDesignOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={AntDesignOutlinedSvg} />;

 /**![ant-design](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(AntDesignOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'AntDesignOutlined';
}

export default RefIcon;