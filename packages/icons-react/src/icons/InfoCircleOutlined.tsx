// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import InfoCircleOutlinedSvg from '@ant-design/icons-svg/asn/InfoCircleOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const InfoCircleOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={InfoCircleOutlinedSvg} />;

 /**![info-circle](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(InfoCircleOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'InfoCircleOutlined';
}

export default RefIcon;