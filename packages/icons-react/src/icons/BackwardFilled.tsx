// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import BackwardFilledSvg from '@ant-design/icons-svg/asn/BackwardFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const BackwardFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={BackwardFilledSvg} />;

 /**![backward](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(BackwardFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'BackwardFilled';
}

export default RefIcon;