// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import WalletTwoToneSvg from '@ant-design/icons-svg/asn/WalletTwoTone';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const WalletTwoTone = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={WalletTwoToneSvg} />;

 /**![wallet](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(WalletTwoTone);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'WalletTwoTone';
}

export default RefIcon;