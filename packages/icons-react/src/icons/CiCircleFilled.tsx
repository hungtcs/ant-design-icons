// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import CiCircleFilledSvg from '@ant-design/icons-svg/asn/CiCircleFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const CiCircleFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={CiCircleFilledSvg} />;

 /**![ci-circle](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(CiCircleFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'CiCircleFilled';
}

export default RefIcon;