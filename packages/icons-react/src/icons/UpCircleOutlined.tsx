// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import UpCircleOutlinedSvg from '@ant-design/icons-svg/asn/UpCircleOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const UpCircleOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={UpCircleOutlinedSvg} />;

 /**![up-circle](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(UpCircleOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'UpCircleOutlined';
}

export default RefIcon;