// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import ClockCircleOutlinedSvg from '@ant-design/icons-svg/asn/ClockCircleOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const ClockCircleOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={ClockCircleOutlinedSvg} />;

 /**![clock-circle](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(ClockCircleOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'ClockCircleOutlined';
}

export default RefIcon;