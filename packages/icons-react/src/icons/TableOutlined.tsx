// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import TableOutlinedSvg from '@ant-design/icons-svg/asn/TableOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const TableOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={TableOutlinedSvg} />;

 /**![table](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(TableOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'TableOutlined';
}

export default RefIcon;