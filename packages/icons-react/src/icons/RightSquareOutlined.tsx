// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import RightSquareOutlinedSvg from '@ant-design/icons-svg/asn/RightSquareOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const RightSquareOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={RightSquareOutlinedSvg} />;

 /**![right-square](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(RightSquareOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'RightSquareOutlined';
}

export default RefIcon;