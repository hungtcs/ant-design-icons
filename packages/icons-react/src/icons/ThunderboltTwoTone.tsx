// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import ThunderboltTwoToneSvg from '@ant-design/icons-svg/asn/ThunderboltTwoTone';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const ThunderboltTwoTone = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={ThunderboltTwoToneSvg} />;

 /**![thunderbolt](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(ThunderboltTwoTone);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'ThunderboltTwoTone';
}

export default RefIcon;