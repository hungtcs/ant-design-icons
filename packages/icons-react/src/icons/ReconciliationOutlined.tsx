// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import ReconciliationOutlinedSvg from '@ant-design/icons-svg/asn/ReconciliationOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const ReconciliationOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={ReconciliationOutlinedSvg} />;

 /**![reconciliation](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(ReconciliationOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'ReconciliationOutlined';
}

export default RefIcon;