// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import ExperimentOutlinedSvg from '@ant-design/icons-svg/asn/ExperimentOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const ExperimentOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={ExperimentOutlinedSvg} />;

 /**![experiment](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(ExperimentOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'ExperimentOutlined';
}

export default RefIcon;