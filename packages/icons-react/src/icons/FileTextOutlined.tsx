// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FileTextOutlinedSvg from '@ant-design/icons-svg/asn/FileTextOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FileTextOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FileTextOutlinedSvg} />;

 /**![file-text](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FileTextOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FileTextOutlined';
}

export default RefIcon;