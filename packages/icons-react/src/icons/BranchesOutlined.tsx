// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import BranchesOutlinedSvg from '@ant-design/icons-svg/asn/BranchesOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const BranchesOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={BranchesOutlinedSvg} />;

 /**![branches](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(BranchesOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'BranchesOutlined';
}

export default RefIcon;