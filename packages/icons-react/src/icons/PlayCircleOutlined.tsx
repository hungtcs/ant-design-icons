// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import PlayCircleOutlinedSvg from '@ant-design/icons-svg/asn/PlayCircleOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const PlayCircleOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={PlayCircleOutlinedSvg} />;

 /**![play-circle](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(PlayCircleOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'PlayCircleOutlined';
}

export default RefIcon;