// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import BlockOutlinedSvg from '@ant-design/icons-svg/asn/BlockOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const BlockOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={BlockOutlinedSvg} />;

 /**![block](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(BlockOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'BlockOutlined';
}

export default RefIcon;