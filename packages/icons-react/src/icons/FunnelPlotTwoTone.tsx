// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FunnelPlotTwoToneSvg from '@ant-design/icons-svg/asn/FunnelPlotTwoTone';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FunnelPlotTwoTone = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FunnelPlotTwoToneSvg} />;

 /**![funnel-plot](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FunnelPlotTwoTone);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FunnelPlotTwoTone';
}

export default RefIcon;