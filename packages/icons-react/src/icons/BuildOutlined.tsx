// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import BuildOutlinedSvg from '@ant-design/icons-svg/asn/BuildOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const BuildOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={BuildOutlinedSvg} />;

 /**![build](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(BuildOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'BuildOutlined';
}

export default RefIcon;