// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import DeliveredProcedureOutlinedSvg from '@ant-design/icons-svg/asn/DeliveredProcedureOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const DeliveredProcedureOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={DeliveredProcedureOutlinedSvg} />;

 /**![delivered-procedure](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(DeliveredProcedureOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'DeliveredProcedureOutlined';
}

export default RefIcon;