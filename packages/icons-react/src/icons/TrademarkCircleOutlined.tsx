// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import TrademarkCircleOutlinedSvg from '@ant-design/icons-svg/asn/TrademarkCircleOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const TrademarkCircleOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={TrademarkCircleOutlinedSvg} />;

 /**![trademark-circle](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(TrademarkCircleOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'TrademarkCircleOutlined';
}

export default RefIcon;