// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import AppstoreOutlinedSvg from '@ant-design/icons-svg/asn/AppstoreOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const AppstoreOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={AppstoreOutlinedSvg} />;

 /**![appstore](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(AppstoreOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'AppstoreOutlined';
}

export default RefIcon;