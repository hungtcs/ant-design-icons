// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import SecurityScanFilledSvg from '@ant-design/icons-svg/asn/SecurityScanFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const SecurityScanFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={SecurityScanFilledSvg} />;

 /**![security-scan](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(SecurityScanFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'SecurityScanFilled';
}

export default RefIcon;