// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import NodeExpandOutlinedSvg from '@ant-design/icons-svg/asn/NodeExpandOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const NodeExpandOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={NodeExpandOutlinedSvg} />;

 /**![node-expand](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(NodeExpandOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'NodeExpandOutlined';
}

export default RefIcon;