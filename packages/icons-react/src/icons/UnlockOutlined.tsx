// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import UnlockOutlinedSvg from '@ant-design/icons-svg/asn/UnlockOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const UnlockOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={UnlockOutlinedSvg} />;

 /**![unlock](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(UnlockOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'UnlockOutlined';
}

export default RefIcon;