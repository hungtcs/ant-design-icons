// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FileAddFilledSvg from '@ant-design/icons-svg/asn/FileAddFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FileAddFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FileAddFilledSvg} />;

 /**![file-add](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FileAddFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FileAddFilled';
}

export default RefIcon;