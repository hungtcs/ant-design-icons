// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import WarningOutlinedSvg from '@ant-design/icons-svg/asn/WarningOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const WarningOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={WarningOutlinedSvg} />;

 /**![warning](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(WarningOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'WarningOutlined';
}

export default RefIcon;