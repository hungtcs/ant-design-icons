// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FacebookOutlinedSvg from '@ant-design/icons-svg/asn/FacebookOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FacebookOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FacebookOutlinedSvg} />;

 /**![facebook](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FacebookOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FacebookOutlined';
}

export default RefIcon;