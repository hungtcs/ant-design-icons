// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import PrinterFilledSvg from '@ant-design/icons-svg/asn/PrinterFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const PrinterFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={PrinterFilledSvg} />;

 /**![printer](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(PrinterFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'PrinterFilled';
}

export default RefIcon;