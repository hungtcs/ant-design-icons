// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import LeftSquareTwoToneSvg from '@ant-design/icons-svg/asn/LeftSquareTwoTone';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const LeftSquareTwoTone = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={LeftSquareTwoToneSvg} />;

 /**![left-square](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(LeftSquareTwoTone);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'LeftSquareTwoTone';
}

export default RefIcon;