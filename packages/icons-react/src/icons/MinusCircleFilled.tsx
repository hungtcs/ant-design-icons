// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import MinusCircleFilledSvg from '@ant-design/icons-svg/asn/MinusCircleFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const MinusCircleFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={MinusCircleFilledSvg} />;

 /**![minus-circle](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(MinusCircleFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'MinusCircleFilled';
}

export default RefIcon;