// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import MinusSquareOutlinedSvg from '@ant-design/icons-svg/asn/MinusSquareOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const MinusSquareOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={MinusSquareOutlinedSvg} />;

 /**![minus-square](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(MinusSquareOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'MinusSquareOutlined';
}

export default RefIcon;