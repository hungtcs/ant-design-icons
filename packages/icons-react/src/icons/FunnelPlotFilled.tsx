// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FunnelPlotFilledSvg from '@ant-design/icons-svg/asn/FunnelPlotFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FunnelPlotFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FunnelPlotFilledSvg} />;

 /**![funnel-plot](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FunnelPlotFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FunnelPlotFilled';
}

export default RefIcon;