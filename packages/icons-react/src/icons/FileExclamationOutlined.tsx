// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FileExclamationOutlinedSvg from '@ant-design/icons-svg/asn/FileExclamationOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FileExclamationOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FileExclamationOutlinedSvg} />;

 /**![file-exclamation](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FileExclamationOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FileExclamationOutlined';
}

export default RefIcon;