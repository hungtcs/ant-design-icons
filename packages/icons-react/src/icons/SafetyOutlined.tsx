// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import SafetyOutlinedSvg from '@ant-design/icons-svg/asn/SafetyOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const SafetyOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={SafetyOutlinedSvg} />;

 /**![safety](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(SafetyOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'SafetyOutlined';
}

export default RefIcon;