// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import TrademarkCircleFilledSvg from '@ant-design/icons-svg/asn/TrademarkCircleFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const TrademarkCircleFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={TrademarkCircleFilledSvg} />;

 /**![trademark-circle](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(TrademarkCircleFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'TrademarkCircleFilled';
}

export default RefIcon;