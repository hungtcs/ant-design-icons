// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import ExpandOutlinedSvg from '@ant-design/icons-svg/asn/ExpandOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const ExpandOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={ExpandOutlinedSvg} />;

 /**![expand](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(ExpandOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'ExpandOutlined';
}

export default RefIcon;