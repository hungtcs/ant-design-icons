// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import CloudOutlinedSvg from '@ant-design/icons-svg/asn/CloudOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const CloudOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={CloudOutlinedSvg} />;

 /**![cloud](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(CloudOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'CloudOutlined';
}

export default RefIcon;