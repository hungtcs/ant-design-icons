// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import SketchSquareFilledSvg from '@ant-design/icons-svg/asn/SketchSquareFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const SketchSquareFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={SketchSquareFilledSvg} />;

 /**![sketch-square](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(SketchSquareFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'SketchSquareFilled';
}

export default RefIcon;