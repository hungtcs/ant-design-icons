// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import CalendarTwoToneSvg from '@ant-design/icons-svg/asn/CalendarTwoTone';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const CalendarTwoTone = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={CalendarTwoToneSvg} />;

 /**![calendar](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(CalendarTwoTone);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'CalendarTwoTone';
}

export default RefIcon;