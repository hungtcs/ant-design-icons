// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import DeleteTwoToneSvg from '@ant-design/icons-svg/asn/DeleteTwoTone';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const DeleteTwoTone = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={DeleteTwoToneSvg} />;

 /**![delete](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(DeleteTwoTone);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'DeleteTwoTone';
}

export default RefIcon;