// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import UpSquareTwoToneSvg from '@ant-design/icons-svg/asn/UpSquareTwoTone';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const UpSquareTwoTone = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={UpSquareTwoToneSvg} />;

 /**![up-square](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(UpSquareTwoTone);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'UpSquareTwoTone';
}

export default RefIcon;