// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import RightSquareFilledSvg from '@ant-design/icons-svg/asn/RightSquareFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const RightSquareFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={RightSquareFilledSvg} />;

 /**![right-square](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(RightSquareFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'RightSquareFilled';
}

export default RefIcon;