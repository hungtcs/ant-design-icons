// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import ForwardOutlinedSvg from '@ant-design/icons-svg/asn/ForwardOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const ForwardOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={ForwardOutlinedSvg} />;

 /**![forward](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(ForwardOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'ForwardOutlined';
}

export default RefIcon;