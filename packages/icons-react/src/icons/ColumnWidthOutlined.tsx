// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import ColumnWidthOutlinedSvg from '@ant-design/icons-svg/asn/ColumnWidthOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const ColumnWidthOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={ColumnWidthOutlinedSvg} />;

 /**![column-width](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(ColumnWidthOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'ColumnWidthOutlined';
}

export default RefIcon;