// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FacebookFilledSvg from '@ant-design/icons-svg/asn/FacebookFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FacebookFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FacebookFilledSvg} />;

 /**![facebook](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FacebookFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FacebookFilled';
}

export default RefIcon;