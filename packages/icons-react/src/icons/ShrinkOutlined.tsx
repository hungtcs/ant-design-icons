// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import ShrinkOutlinedSvg from '@ant-design/icons-svg/asn/ShrinkOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const ShrinkOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={ShrinkOutlinedSvg} />;

 /**![shrink](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(ShrinkOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'ShrinkOutlined';
}

export default RefIcon;