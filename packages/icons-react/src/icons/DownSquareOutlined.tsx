// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import DownSquareOutlinedSvg from '@ant-design/icons-svg/asn/DownSquareOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const DownSquareOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={DownSquareOutlinedSvg} />;

 /**![down-square](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(DownSquareOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'DownSquareOutlined';
}

export default RefIcon;