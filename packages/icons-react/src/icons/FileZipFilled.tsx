// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FileZipFilledSvg from '@ant-design/icons-svg/asn/FileZipFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FileZipFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FileZipFilledSvg} />;

 /**![file-zip](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FileZipFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FileZipFilled';
}

export default RefIcon;