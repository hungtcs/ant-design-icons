// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import CustomerServiceFilledSvg from '@ant-design/icons-svg/asn/CustomerServiceFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const CustomerServiceFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={CustomerServiceFilledSvg} />;

 /**![customer-service](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(CustomerServiceFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'CustomerServiceFilled';
}

export default RefIcon;