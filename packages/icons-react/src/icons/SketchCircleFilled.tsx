// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import SketchCircleFilledSvg from '@ant-design/icons-svg/asn/SketchCircleFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const SketchCircleFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={SketchCircleFilledSvg} />;

 /**![sketch-circle](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(SketchCircleFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'SketchCircleFilled';
}

export default RefIcon;