// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import EyeInvisibleFilledSvg from '@ant-design/icons-svg/asn/EyeInvisibleFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const EyeInvisibleFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={EyeInvisibleFilledSvg} />;

 /**![eye-invisible](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(EyeInvisibleFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'EyeInvisibleFilled';
}

export default RefIcon;