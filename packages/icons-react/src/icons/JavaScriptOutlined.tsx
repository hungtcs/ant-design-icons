// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import JavaScriptOutlinedSvg from '@ant-design/icons-svg/asn/JavaScriptOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const JavaScriptOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={JavaScriptOutlinedSvg} />;

 /**![java-script](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(JavaScriptOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'JavaScriptOutlined';
}

export default RefIcon;