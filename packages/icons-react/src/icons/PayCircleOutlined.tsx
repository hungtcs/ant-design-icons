// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import PayCircleOutlinedSvg from '@ant-design/icons-svg/asn/PayCircleOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const PayCircleOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={PayCircleOutlinedSvg} />;

 /**![pay-circle](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(PayCircleOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'PayCircleOutlined';
}

export default RefIcon;