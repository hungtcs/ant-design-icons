// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FilterOutlinedSvg from '@ant-design/icons-svg/asn/FilterOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FilterOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FilterOutlinedSvg} />;

 /**![filter](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FilterOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FilterOutlined';
}

export default RefIcon;