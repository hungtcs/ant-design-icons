// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import VideoCameraOutlinedSvg from '@ant-design/icons-svg/asn/VideoCameraOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const VideoCameraOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={VideoCameraOutlinedSvg} />;

 /**![video-camera](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(VideoCameraOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'VideoCameraOutlined';
}

export default RefIcon;