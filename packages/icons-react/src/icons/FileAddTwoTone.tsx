// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FileAddTwoToneSvg from '@ant-design/icons-svg/asn/FileAddTwoTone';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FileAddTwoTone = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FileAddTwoToneSvg} />;

 /**![file-add](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FileAddTwoTone);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FileAddTwoTone';
}

export default RefIcon;