// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import PushpinTwoToneSvg from '@ant-design/icons-svg/asn/PushpinTwoTone';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const PushpinTwoTone = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={PushpinTwoToneSvg} />;

 /**![pushpin](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(PushpinTwoTone);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'PushpinTwoTone';
}

export default RefIcon;