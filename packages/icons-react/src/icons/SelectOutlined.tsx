// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import SelectOutlinedSvg from '@ant-design/icons-svg/asn/SelectOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const SelectOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={SelectOutlinedSvg} />;

 /**![select](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(SelectOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'SelectOutlined';
}

export default RefIcon;