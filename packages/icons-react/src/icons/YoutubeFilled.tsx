// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import YoutubeFilledSvg from '@ant-design/icons-svg/asn/YoutubeFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const YoutubeFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={YoutubeFilledSvg} />;

 /**![youtube](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(YoutubeFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'YoutubeFilled';
}

export default RefIcon;