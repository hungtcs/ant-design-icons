// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FolderOutlinedSvg from '@ant-design/icons-svg/asn/FolderOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FolderOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FolderOutlinedSvg} />;

 /**![folder](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FolderOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FolderOutlined';
}

export default RefIcon;