// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import UnorderedListOutlinedSvg from '@ant-design/icons-svg/asn/UnorderedListOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const UnorderedListOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={UnorderedListOutlinedSvg} />;

 /**![unordered-list](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(UnorderedListOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'UnorderedListOutlined';
}

export default RefIcon;