// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import MinusOutlinedSvg from '@ant-design/icons-svg/asn/MinusOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const MinusOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={MinusOutlinedSvg} />;

 /**![minus](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(MinusOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'MinusOutlined';
}

export default RefIcon;