// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import UnderlineOutlinedSvg from '@ant-design/icons-svg/asn/UnderlineOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const UnderlineOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={UnderlineOutlinedSvg} />;

 /**![underline](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(UnderlineOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'UnderlineOutlined';
}

export default RefIcon;