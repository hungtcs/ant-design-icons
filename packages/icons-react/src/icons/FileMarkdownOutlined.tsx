// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FileMarkdownOutlinedSvg from '@ant-design/icons-svg/asn/FileMarkdownOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FileMarkdownOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FileMarkdownOutlinedSvg} />;

 /**![file-markdown](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FileMarkdownOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FileMarkdownOutlined';
}

export default RefIcon;