// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import CheckCircleOutlinedSvg from '@ant-design/icons-svg/asn/CheckCircleOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const CheckCircleOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={CheckCircleOutlinedSvg} />;

 /**![check-circle](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(CheckCircleOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'CheckCircleOutlined';
}

export default RefIcon;