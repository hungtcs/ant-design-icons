// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import ShakeOutlinedSvg from '@ant-design/icons-svg/asn/ShakeOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const ShakeOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={ShakeOutlinedSvg} />;

 /**![shake](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(ShakeOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'ShakeOutlined';
}

export default RefIcon;