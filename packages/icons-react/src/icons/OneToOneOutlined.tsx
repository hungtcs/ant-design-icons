// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import OneToOneOutlinedSvg from '@ant-design/icons-svg/asn/OneToOneOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const OneToOneOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={OneToOneOutlinedSvg} />;

 /**![one-to-one](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(OneToOneOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'OneToOneOutlined';
}

export default RefIcon;