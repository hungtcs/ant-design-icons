// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FileTextFilledSvg from '@ant-design/icons-svg/asn/FileTextFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FileTextFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FileTextFilledSvg} />;

 /**![file-text](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FileTextFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FileTextFilled';
}

export default RefIcon;