// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import StepForwardOutlinedSvg from '@ant-design/icons-svg/asn/StepForwardOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const StepForwardOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={StepForwardOutlinedSvg} />;

 /**![step-forward](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(StepForwardOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'StepForwardOutlined';
}

export default RefIcon;