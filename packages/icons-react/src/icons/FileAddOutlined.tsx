// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FileAddOutlinedSvg from '@ant-design/icons-svg/asn/FileAddOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FileAddOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FileAddOutlinedSvg} />;

 /**![file-add](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FileAddOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FileAddOutlined';
}

export default RefIcon;