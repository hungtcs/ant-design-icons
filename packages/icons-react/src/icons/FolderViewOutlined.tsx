// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FolderViewOutlinedSvg from '@ant-design/icons-svg/asn/FolderViewOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FolderViewOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FolderViewOutlinedSvg} />;

 /**![folder-view](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FolderViewOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FolderViewOutlined';
}

export default RefIcon;