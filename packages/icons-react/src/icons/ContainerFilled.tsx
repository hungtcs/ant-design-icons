// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import ContainerFilledSvg from '@ant-design/icons-svg/asn/ContainerFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const ContainerFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={ContainerFilledSvg} />;

 /**![container](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(ContainerFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'ContainerFilled';
}

export default RefIcon;