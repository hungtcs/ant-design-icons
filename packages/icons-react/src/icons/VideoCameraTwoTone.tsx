// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import VideoCameraTwoToneSvg from '@ant-design/icons-svg/asn/VideoCameraTwoTone';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const VideoCameraTwoTone = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={VideoCameraTwoToneSvg} />;

 /**![video-camera](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(VideoCameraTwoTone);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'VideoCameraTwoTone';
}

export default RefIcon;