// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FileImageFilledSvg from '@ant-design/icons-svg/asn/FileImageFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FileImageFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FileImageFilledSvg} />;

 /**![file-image](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FileImageFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FileImageFilled';
}

export default RefIcon;