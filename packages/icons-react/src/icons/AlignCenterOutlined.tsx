// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import AlignCenterOutlinedSvg from '@ant-design/icons-svg/asn/AlignCenterOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const AlignCenterOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={AlignCenterOutlinedSvg} />;

 /**![align-center](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(AlignCenterOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'AlignCenterOutlined';
}

export default RefIcon;