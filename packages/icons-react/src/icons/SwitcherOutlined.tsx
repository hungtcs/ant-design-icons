// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import SwitcherOutlinedSvg from '@ant-design/icons-svg/asn/SwitcherOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const SwitcherOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={SwitcherOutlinedSvg} />;

 /**![switcher](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(SwitcherOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'SwitcherOutlined';
}

export default RefIcon;