// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import DesktopOutlinedSvg from '@ant-design/icons-svg/asn/DesktopOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const DesktopOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={DesktopOutlinedSvg} />;

 /**![desktop](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(DesktopOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'DesktopOutlined';
}

export default RefIcon;