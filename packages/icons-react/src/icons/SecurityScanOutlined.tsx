// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import SecurityScanOutlinedSvg from '@ant-design/icons-svg/asn/SecurityScanOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const SecurityScanOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={SecurityScanOutlinedSvg} />;

 /**![security-scan](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(SecurityScanOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'SecurityScanOutlined';
}

export default RefIcon;