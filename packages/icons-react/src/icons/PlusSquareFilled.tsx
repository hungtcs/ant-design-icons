// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import PlusSquareFilledSvg from '@ant-design/icons-svg/asn/PlusSquareFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const PlusSquareFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={PlusSquareFilledSvg} />;

 /**![plus-square](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(PlusSquareFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'PlusSquareFilled';
}

export default RefIcon;