// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import PictureOutlinedSvg from '@ant-design/icons-svg/asn/PictureOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const PictureOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={PictureOutlinedSvg} />;

 /**![picture](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(PictureOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'PictureOutlined';
}

export default RefIcon;