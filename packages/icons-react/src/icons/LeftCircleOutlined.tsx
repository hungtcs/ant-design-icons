// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import LeftCircleOutlinedSvg from '@ant-design/icons-svg/asn/LeftCircleOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const LeftCircleOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={LeftCircleOutlinedSvg} />;

 /**![left-circle](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(LeftCircleOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'LeftCircleOutlined';
}

export default RefIcon;