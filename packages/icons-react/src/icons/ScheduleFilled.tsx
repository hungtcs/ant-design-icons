// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import ScheduleFilledSvg from '@ant-design/icons-svg/asn/ScheduleFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const ScheduleFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={ScheduleFilledSvg} />;

 /**![schedule](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(ScheduleFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'ScheduleFilled';
}

export default RefIcon;