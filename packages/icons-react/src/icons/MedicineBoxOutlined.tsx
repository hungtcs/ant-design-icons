// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import MedicineBoxOutlinedSvg from '@ant-design/icons-svg/asn/MedicineBoxOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const MedicineBoxOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={MedicineBoxOutlinedSvg} />;

 /**![medicine-box](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(MedicineBoxOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'MedicineBoxOutlined';
}

export default RefIcon;