// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import ThunderboltFilledSvg from '@ant-design/icons-svg/asn/ThunderboltFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const ThunderboltFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={ThunderboltFilledSvg} />;

 /**![thunderbolt](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(ThunderboltFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'ThunderboltFilled';
}

export default RefIcon;