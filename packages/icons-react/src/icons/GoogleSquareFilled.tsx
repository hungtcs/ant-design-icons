// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import GoogleSquareFilledSvg from '@ant-design/icons-svg/asn/GoogleSquareFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const GoogleSquareFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={GoogleSquareFilledSvg} />;

 /**![google-square](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(GoogleSquareFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'GoogleSquareFilled';
}

export default RefIcon;