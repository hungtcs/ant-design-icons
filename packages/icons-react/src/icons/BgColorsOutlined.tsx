// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import BgColorsOutlinedSvg from '@ant-design/icons-svg/asn/BgColorsOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const BgColorsOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={BgColorsOutlinedSvg} />;

 /**![bg-colors](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(BgColorsOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'BgColorsOutlined';
}

export default RefIcon;