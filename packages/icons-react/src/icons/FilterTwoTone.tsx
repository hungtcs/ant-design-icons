// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FilterTwoToneSvg from '@ant-design/icons-svg/asn/FilterTwoTone';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FilterTwoTone = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FilterTwoToneSvg} />;

 /**![filter](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FilterTwoTone);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FilterTwoTone';
}

export default RefIcon;