// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import CompassOutlinedSvg from '@ant-design/icons-svg/asn/CompassOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const CompassOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={CompassOutlinedSvg} />;

 /**![compass](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(CompassOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'CompassOutlined';
}

export default RefIcon;