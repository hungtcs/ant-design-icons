// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import SwitcherTwoToneSvg from '@ant-design/icons-svg/asn/SwitcherTwoTone';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const SwitcherTwoTone = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={SwitcherTwoToneSvg} />;

 /**![switcher](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(SwitcherTwoTone);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'SwitcherTwoTone';
}

export default RefIcon;