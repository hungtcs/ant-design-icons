// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import CreditCardOutlinedSvg from '@ant-design/icons-svg/asn/CreditCardOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const CreditCardOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={CreditCardOutlinedSvg} />;

 /**![credit-card](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(CreditCardOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'CreditCardOutlined';
}

export default RefIcon;