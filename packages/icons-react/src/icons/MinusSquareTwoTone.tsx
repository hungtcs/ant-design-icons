// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import MinusSquareTwoToneSvg from '@ant-design/icons-svg/asn/MinusSquareTwoTone';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const MinusSquareTwoTone = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={MinusSquareTwoToneSvg} />;

 /**![minus-square](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(MinusSquareTwoTone);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'MinusSquareTwoTone';
}

export default RefIcon;