// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import RetweetOutlinedSvg from '@ant-design/icons-svg/asn/RetweetOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const RetweetOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={RetweetOutlinedSvg} />;

 /**![retweet](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(RetweetOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'RetweetOutlined';
}

export default RefIcon;