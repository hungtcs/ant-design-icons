// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import TrademarkOutlinedSvg from '@ant-design/icons-svg/asn/TrademarkOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const TrademarkOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={TrademarkOutlinedSvg} />;

 /**![trademark](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(TrademarkOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'TrademarkOutlined';
}

export default RefIcon;