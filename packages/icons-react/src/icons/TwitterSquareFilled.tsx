// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import TwitterSquareFilledSvg from '@ant-design/icons-svg/asn/TwitterSquareFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const TwitterSquareFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={TwitterSquareFilledSvg} />;

 /**![twitter-square](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(TwitterSquareFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'TwitterSquareFilled';
}

export default RefIcon;