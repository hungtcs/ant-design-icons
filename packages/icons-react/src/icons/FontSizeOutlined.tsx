// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FontSizeOutlinedSvg from '@ant-design/icons-svg/asn/FontSizeOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FontSizeOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FontSizeOutlinedSvg} />;

 /**![font-size](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FontSizeOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FontSizeOutlined';
}

export default RefIcon;