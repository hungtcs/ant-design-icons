// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import CodeSandboxCircleFilledSvg from '@ant-design/icons-svg/asn/CodeSandboxCircleFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const CodeSandboxCircleFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={CodeSandboxCircleFilledSvg} />;

 /**![code-sandbox-circle](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(CodeSandboxCircleFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'CodeSandboxCircleFilled';
}

export default RefIcon;