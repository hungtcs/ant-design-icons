// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import HolderOutlinedSvg from '@ant-design/icons-svg/asn/HolderOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const HolderOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={HolderOutlinedSvg} />;

 /**![holder](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(HolderOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'HolderOutlined';
}

export default RefIcon;