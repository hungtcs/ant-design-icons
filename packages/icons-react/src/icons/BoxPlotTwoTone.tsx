// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import BoxPlotTwoToneSvg from '@ant-design/icons-svg/asn/BoxPlotTwoTone';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const BoxPlotTwoTone = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={BoxPlotTwoToneSvg} />;

 /**![box-plot](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(BoxPlotTwoTone);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'BoxPlotTwoTone';
}

export default RefIcon;