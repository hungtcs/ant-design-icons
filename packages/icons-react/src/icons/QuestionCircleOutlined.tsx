// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import QuestionCircleOutlinedSvg from '@ant-design/icons-svg/asn/QuestionCircleOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const QuestionCircleOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={QuestionCircleOutlinedSvg} />;

 /**![question-circle](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(QuestionCircleOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'QuestionCircleOutlined';
}

export default RefIcon;