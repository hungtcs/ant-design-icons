// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import BoxPlotOutlinedSvg from '@ant-design/icons-svg/asn/BoxPlotOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const BoxPlotOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={BoxPlotOutlinedSvg} />;

 /**![box-plot](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(BoxPlotOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'BoxPlotOutlined';
}

export default RefIcon;