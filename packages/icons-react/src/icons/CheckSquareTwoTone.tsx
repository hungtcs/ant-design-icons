// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import CheckSquareTwoToneSvg from '@ant-design/icons-svg/asn/CheckSquareTwoTone';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const CheckSquareTwoTone = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={CheckSquareTwoToneSvg} />;

 /**![check-square](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(CheckSquareTwoTone);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'CheckSquareTwoTone';
}

export default RefIcon;