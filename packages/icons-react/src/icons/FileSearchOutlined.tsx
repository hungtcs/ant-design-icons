// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FileSearchOutlinedSvg from '@ant-design/icons-svg/asn/FileSearchOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FileSearchOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FileSearchOutlinedSvg} />;

 /**![file-search](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FileSearchOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FileSearchOutlined';
}

export default RefIcon;