// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import TranslationOutlinedSvg from '@ant-design/icons-svg/asn/TranslationOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const TranslationOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={TranslationOutlinedSvg} />;

 /**![translation](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(TranslationOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'TranslationOutlined';
}

export default RefIcon;