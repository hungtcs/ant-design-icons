// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import CarryOutFilledSvg from '@ant-design/icons-svg/asn/CarryOutFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const CarryOutFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={CarryOutFilledSvg} />;

 /**![carry-out](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(CarryOutFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'CarryOutFilled';
}

export default RefIcon;