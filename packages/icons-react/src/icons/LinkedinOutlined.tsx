// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import LinkedinOutlinedSvg from '@ant-design/icons-svg/asn/LinkedinOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const LinkedinOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={LinkedinOutlinedSvg} />;

 /**![linkedin](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(LinkedinOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'LinkedinOutlined';
}

export default RefIcon;