// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import ProjectFilledSvg from '@ant-design/icons-svg/asn/ProjectFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const ProjectFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={ProjectFilledSvg} />;

 /**![project](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(ProjectFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'ProjectFilled';
}

export default RefIcon;