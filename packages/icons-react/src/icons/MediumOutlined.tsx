// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import MediumOutlinedSvg from '@ant-design/icons-svg/asn/MediumOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const MediumOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={MediumOutlinedSvg} />;

 /**![medium](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(MediumOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'MediumOutlined';
}

export default RefIcon;