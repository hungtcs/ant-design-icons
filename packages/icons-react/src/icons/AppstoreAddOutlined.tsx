// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import AppstoreAddOutlinedSvg from '@ant-design/icons-svg/asn/AppstoreAddOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const AppstoreAddOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={AppstoreAddOutlinedSvg} />;

 /**![appstore-add](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(AppstoreAddOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'AppstoreAddOutlined';
}

export default RefIcon;