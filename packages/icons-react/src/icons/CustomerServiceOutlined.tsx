// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import CustomerServiceOutlinedSvg from '@ant-design/icons-svg/asn/CustomerServiceOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const CustomerServiceOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={CustomerServiceOutlinedSvg} />;

 /**![customer-service](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(CustomerServiceOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'CustomerServiceOutlined';
}

export default RefIcon;