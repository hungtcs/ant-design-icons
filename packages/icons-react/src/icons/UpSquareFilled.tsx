// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import UpSquareFilledSvg from '@ant-design/icons-svg/asn/UpSquareFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const UpSquareFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={UpSquareFilledSvg} />;

 /**![up-square](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(UpSquareFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'UpSquareFilled';
}

export default RefIcon;