// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import TrophyOutlinedSvg from '@ant-design/icons-svg/asn/TrophyOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const TrophyOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={TrophyOutlinedSvg} />;

 /**![trophy](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(TrophyOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'TrophyOutlined';
}

export default RefIcon;