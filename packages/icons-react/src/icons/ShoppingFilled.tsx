// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import ShoppingFilledSvg from '@ant-design/icons-svg/asn/ShoppingFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const ShoppingFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={ShoppingFilledSvg} />;

 /**![shopping](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(ShoppingFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'ShoppingFilled';
}

export default RefIcon;