// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FileMarkdownFilledSvg from '@ant-design/icons-svg/asn/FileMarkdownFilled';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FileMarkdownFilled = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FileMarkdownFilledSvg} />;

 /**![file-markdown](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FileMarkdownFilled);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FileMarkdownFilled';
}

export default RefIcon;