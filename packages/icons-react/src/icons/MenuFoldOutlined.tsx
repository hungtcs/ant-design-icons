// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import MenuFoldOutlinedSvg from '@ant-design/icons-svg/asn/MenuFoldOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const MenuFoldOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={MenuFoldOutlinedSvg} />;

 /**![menu-fold](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(MenuFoldOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'MenuFoldOutlined';
}

export default RefIcon;