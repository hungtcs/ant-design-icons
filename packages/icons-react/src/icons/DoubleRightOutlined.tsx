// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import DoubleRightOutlinedSvg from '@ant-design/icons-svg/asn/DoubleRightOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const DoubleRightOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={DoubleRightOutlinedSvg} />;

 /**![double-right](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(DoubleRightOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'DoubleRightOutlined';
}

export default RefIcon;