// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FileImageOutlinedSvg from '@ant-design/icons-svg/asn/FileImageOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FileImageOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FileImageOutlinedSvg} />;

 /**![file-image](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FileImageOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FileImageOutlined';
}

export default RefIcon;