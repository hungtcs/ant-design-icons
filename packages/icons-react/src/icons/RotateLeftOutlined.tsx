// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import RotateLeftOutlinedSvg from '@ant-design/icons-svg/asn/RotateLeftOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const RotateLeftOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={RotateLeftOutlinedSvg} />;

 /**![rotate-left](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(RotateLeftOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'RotateLeftOutlined';
}

export default RefIcon;