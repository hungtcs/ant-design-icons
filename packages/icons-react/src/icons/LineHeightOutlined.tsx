// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import LineHeightOutlinedSvg from '@ant-design/icons-svg/asn/LineHeightOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const LineHeightOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={LineHeightOutlinedSvg} />;

 /**![line-height](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(LineHeightOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'LineHeightOutlined';
}

export default RefIcon;