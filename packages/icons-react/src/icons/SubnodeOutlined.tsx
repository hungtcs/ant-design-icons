// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import SubnodeOutlinedSvg from '@ant-design/icons-svg/asn/SubnodeOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const SubnodeOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={SubnodeOutlinedSvg} />;

 /**![subnode](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(SubnodeOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'SubnodeOutlined';
}

export default RefIcon;