// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import SafetyCertificateOutlinedSvg from '@ant-design/icons-svg/asn/SafetyCertificateOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const SafetyCertificateOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={SafetyCertificateOutlinedSvg} />;

 /**![safety-certificate](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(SafetyCertificateOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'SafetyCertificateOutlined';
}

export default RefIcon;