// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import NodeCollapseOutlinedSvg from '@ant-design/icons-svg/asn/NodeCollapseOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const NodeCollapseOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={NodeCollapseOutlinedSvg} />;

 /**![node-collapse](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(NodeCollapseOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'NodeCollapseOutlined';
}

export default RefIcon;