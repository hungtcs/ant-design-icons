// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import CreditCardTwoToneSvg from '@ant-design/icons-svg/asn/CreditCardTwoTone';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const CreditCardTwoTone = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={CreditCardTwoToneSvg} />;

 /**![credit-card](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(CreditCardTwoTone);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'CreditCardTwoTone';
}

export default RefIcon;