// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import VerticalLeftOutlinedSvg from '@ant-design/icons-svg/asn/VerticalLeftOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const VerticalLeftOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={VerticalLeftOutlinedSvg} />;

 /**![vertical-left](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(VerticalLeftOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'VerticalLeftOutlined';
}

export default RefIcon;