// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import PicRightOutlinedSvg from '@ant-design/icons-svg/asn/PicRightOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const PicRightOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={PicRightOutlinedSvg} />;

 /**![pic-right](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(PicRightOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'PicRightOutlined';
}

export default RefIcon;