// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import MutedOutlinedSvg from '@ant-design/icons-svg/asn/MutedOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const MutedOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={MutedOutlinedSvg} />;

 /**![muted](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(MutedOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'MutedOutlined';
}

export default RefIcon;