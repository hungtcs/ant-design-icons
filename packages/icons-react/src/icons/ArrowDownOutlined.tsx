// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import ArrowDownOutlinedSvg from '@ant-design/icons-svg/asn/ArrowDownOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const ArrowDownOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={ArrowDownOutlinedSvg} />;

 /**![arrow-down](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(ArrowDownOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'ArrowDownOutlined';
}

export default RefIcon;