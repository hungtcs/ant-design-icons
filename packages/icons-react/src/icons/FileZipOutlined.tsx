// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import * as React from 'react'
import FileZipOutlinedSvg from '@ant-design/icons-svg/asn/FileZipOutlined';
import AntdIcon, { AntdIconProps } from '../components/AntdIcon.js';

const FileZipOutlined = (
  props: AntdIconProps,
  ref: React.MutableRefObject<HTMLSpanElement>,
) => <AntdIcon {...props} ref={ref} icon={FileZipOutlinedSvg} />;

 /**![file-zip](data:image/svg+xml;base64,) */ 
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(FileZipOutlined);

if (process.env.NODE_ENV !== 'production') {
  RefIcon.displayName = 'FileZipOutlined';
}

export default RefIcon;