...
import { IconDefinition } from '@ant-design/icons-svg/types';
...
import <%= svgIdentifier %>Svg from '@ant-design/icons-svg/asn/<%= svgIdentifier %>';
...
const RefIcon = React.forwardRef<HTMLSpanElement, Omit<AntdIconProps, 'ref'>>(<%= svgIdentifier %>);
...
`.trim()
  ...
  const entryText = Object.keys(allIconDefs)
    ...
    .map(
      (svgIdentifier) =>
        `export { default as ${svgIdentifier} } from './${svgIdentifier}.js';`
    )
    .join('\n');
